...
function on_choose_fa_detection(response) {
    if (response === 'Yes') {
        data.result = "The fasp status should be: " + wrap_fasp_status('Detected Screen +ve FA Scan') + ".";
    } else if (response === 'No') {
        data.result = "The fasp status should be: " + wrap_fasp_status('Undetected Screen -ve FA Scan') + ".";
    }
}
...
function on_choose_booked_for_fts(response) {
    if (response === 'Yes') {
        data.result = new Question(
            "Did the pregnancy reach the combined screening window?",
            ["Yes", "No - Miscarriage", "No - Termination Of Pregnancy"],
            on_choose_reached_fts
        )
    } else if (response === 'No') {
...
function on_choose_reached_fts(response) {
    if (response === 'Yes') {
        data.result = new Question(
            "Was the anomaly detected prior to combined screening?",
            ["Yes - NHS", "Yes - Private", "No"],
            on_choose_fts_still_req
        )
    } else if (response === 'No - Miscarriage') {
        data.result = "The fasp status should be: " + wrap_fasp_status('Ineligible: Early fetal loss') + ".";
    } else if (response == 'No - Termination Of Pregnancy') {
        data.result = "The fasp status should be: " + wrap_fasp_status('Ineligible: Early fetal loss (TOP)') + ".";
    }
...
function on_choose_fts_still_req(response) {
    if (response === 'No') {
        data.result = new Question(
...
function on_choose_booked_for_quad(response) {
    ...
    } else if (response === 'Yes - quad declined') {
...
function on_choose_quad_risk(response) {
    if (response === 'Yes') {
        data.result = "The additional fasp status should be: " + wrap_fasp_status('Detected Screen +ve Quad') + ".";
    } else if (response === 'No') {
        data.result = "The additional fasp status should be: " + wrap_fasp_status('Undetected Screen -ve Quad') + ".";
    }
}
...
function on_choose_fts_risk(response) {
    if (response === 'Yes') {
        data.first_screening_status = "The fasp status should be: " + wrap_fasp_status('Detected Screen +ve combined') + ".";
        data.result = "The fasp status should be: " + wrap_fasp_status('Ineligible: Early Diagnosis') + ".";
    } else if (response === 'No') {
        data.first_screening_status = "The fasp status should be: " + wrap_fasp_status('Undetected Screen -ve combined') + ".";
        if (data.anomaly === constants.anomaly_t21) {
            data.result = new Question(
                "Was the pregnancy booked in time for quad screening?",
                ["Yes - quad accepted", "Yes - quad declined", "No"],
